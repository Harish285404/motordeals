...
if (preg_match($arabicPattern, $request->name) && preg_match($arabicPattern, $request->description)) {
    // Both name and description are in Arabic
    $User->arabic_name = $request->name;
    $User->arabic_description = $request->description;
} elseif (!preg_match($arabicPattern, $request->name) && !preg_match($arabicPattern, $request->description)) {
    // Both name and description are in English
    $User->name = $request->name;
    $User->description = $request->description;
    $User->arabic_name = GoogleTranslate::trans($request->name, 'ar');
    $User->arabic_description = GoogleTranslate::trans($request->description, 'ar');
} elseif (preg_match($arabicPattern, $request->name) && !preg_match($arabicPattern, $request->description)) {
    // Name is in Arabic, description is in English
    $User->arabic_name = $request->name;
    $User->arabic_description = GoogleTranslate::trans($request->description, 'ar');
    $User->name = GoogleTranslate::trans($request->name, 'en');
    $User->description = $request->description;
} else {
    // Name is in English, description is in Arabic
    $User->name = $request->name;
    $User->description = GoogleTranslate::trans($request->description, 'en');
    $User->arabic_name = GoogleTranslate::trans($request->name, 'ar');
    $User->arabic_description = $request->description;
}
...
public function product_list()
{
            ...
            // Retrieve data for all tabs
    $sale = Product::where('sell_type', 'Sale')->where('status', '0')->where('category_id', 16)->get();
    $rent = Product::where('sell_type', 'Rent')->where('category_id', 17)->get();
    $bikesale = Product::where('sell_type', 'Sale')->where('status', '0')->where('category_id', 4)->get();
    $partsale = Product::where('sell_type', 'Sale')->where('status', '0')->where('category_id', 5)->get();
...
public function product_search()
{
    ...
    if ($searchTab == "set1") {
        $products = Product::where('sell_type', 'Sale')
            ->where('status', '0')
        ...
        $products = Product::where('sell_type', 'Rent')
            ->where('status', '0')
      ...
      public function edit_product($id)
  {
    ...
    return view('Admin.edit-product', ['product' => $data,'category' => $category,'brand'=>$brand]);
        ...
        public function view_product($id)
  {
    ...
    return view('Admin.view-product', ['products' => $data]);
